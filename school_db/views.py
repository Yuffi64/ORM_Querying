...
def problem_one(request):
    ...
    students__highest_gpa = Student.objects.filter(gpa__gte = 3.0).order_by('-gpa')
    
    for student in students__highest_gpa:
      print(f'First Name: {student.first_name} Last Name: {student.last_name} GPA: {student.gpa}')
...
def problem_two(request):
    ...
    instruc_prior_2010 = Instructor.objects.filter(hire_date__year__lt = 2010).order_by('hire_date')
    
    for instructor in instruc_prior_2010:
